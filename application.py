...
@app.route("/showAllReport")
def showAllReport():
    r = Report.query.all()
    return render_template("show all report.html", report=r)
...
@app.route("/showAllBill")
def showAllBill():
    bills = Bill.query.all()
    return render_template("show all bill.html", bills=bills)


@app.route('/searchBillInfo')
def searchBillInfo():
    return render_template('search bill infor.html')

@app.route('/searchBillInfoCommit')
def searchBillInfoCommit():
    So_HD = request.args.get('So_HD')
    bills = Bill.query.get(So_HD)
    if bills == None :
        return render_template('invalid_id.html')
    return render_template('search bill success.html', bills=bills)
